...
test('rejects empty and version-only files', () => {
  for (const text of ['', "const CARESTEP_VERSION='10.7-A.2.3';\nconst EFSYNC_VERSION='10.7-A.2.3';\n"]) {
    assert.equal(verifyWorkerSource(Buffer.from(text)).ok, false);
  }
});

test('immutable A.2 remains exact and version-only relabeling is not A.2.3', () => {
  ...
  assert.equal(verifyWorkerSource(Buffer.from(text.replaceAll('10.7-A.2', '10.7-A.2.3'))).ok, false);
});
