...
2 FAIL**: clinic ledger collision and old-schema upgrade ordering. See
[the reproduced candidate failures](worker-a2-candidate-review.md).
The remote branch was already synchronized by concurrent commit `c44aeb9`
while this review was running; that source and CI work are preserved. This
review did not approve replacement: its required regression gate failed.
Do not merge or deploy merely because the static CI inventory passes.
...
- Patient/event/map/status/retry/checkpoint/reconcile queries bind the configured
  clinic id, but the new ledger UPSERT does not isolate its conflict key by clinic;
  the reproduced overwrite is a release blocker.
- Run completion UPDATE binds both run id and clinic id.
- The schema uses additive CREATE/ALTER statements, but the consult-status index
  precedes its required column ALTERs and fails on the main schema. No migration
...
First resolve or explicitly review the two reproduced blockers and rerun the
offline regression gate. Any corrected source will have a different hash and
must be documented as a separate reviewed artifact. Only after that gate and
a later explicit merge/deployment approval should a human follow this original
A.2 smoke checklist (updating the source identity if an approved fix changes it):
