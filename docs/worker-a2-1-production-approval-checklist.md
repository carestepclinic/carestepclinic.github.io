...
- [ ] D1 선행 → schema post-check → A.2.1 배포의 조건이 모두 충족됐다: `[M_APPROVAL]`
...
- [ ] A.2.1 유지·영향 경로 제한·증거 보존·forward-fix의 책임자를 지정했다: `[F_OWNER]`
- [ ] 전체 DB 복구 시 백업 이후 데이터 보존/재적용 계획 없이는 실행하지 않는다.
- [ ] 최신 문서 commit의 CI, 36 PASS / 0 FAIL, Worker hash를 재확인했다.
- [ ] 실제 관찰 결과와 미해결 WARN을 검토하고 완료/중단에 서명했다: `[FINAL_DECISION]`

현재 완료된 것은 문서 및 격리 검증뿐이다. Production R/B/M/D/S/F 실행은 모두 대기 상태다.
